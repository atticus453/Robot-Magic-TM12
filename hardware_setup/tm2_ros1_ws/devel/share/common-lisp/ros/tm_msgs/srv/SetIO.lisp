...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<SetIO-request>)))
  ...
  (cl:format cl:nil "# More details please refer to the TM Expression Editor Manual(2.12 rev1.00) Chapter 10.5 IO~%int8 MODULE_CONTROLBOX = 0~%int8 MODULE_ENDEFFECTOR = 1~%~%int8 TYPE_DIGITAL_IN = 0~%int8 TYPE_DIGITAL_OUT = 1~%int8 TYPE_INSTANT_DO = 2~%int8 TYPE_ANALOG_IN = 3~%int8 TYPE_ANALOG_OUT = 4~%int8 TYPE_INSTANT_AO = 5~%~%int8 STATE_OFF = 0~%int8 STATE_ON = 1~%~%# request fields~%int8 module~%int8 type~%int8 pin~%float32 state #STATE_OFF or STATE_ON value, or other value (if type expressed in a specific control module)~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'SetIO-request)))
  "Returns full string definition for message of type 'SetIO-request"
  (cl:format cl:nil "# More details please refer to the TM Expression Editor Manual(2.12 rev1.00) Chapter 10.5 IO~%int8 MODULE_CONTROLBOX = 0~%int8 MODULE_ENDEFFECTOR = 1~%~%int8 TYPE_DIGITAL_IN = 0~%int8 TYPE_DIGITAL_OUT = 1~%int8 TYPE_INSTANT_DO = 2~%int8 TYPE_ANALOG_IN = 3~%int8 TYPE_ANALOG_OUT = 4~%int8 TYPE_INSTANT_AO = 5~%~%int8 STATE_OFF = 0~%int8 STATE_ON = 1~%~%# request fields~%int8 module~%int8 type~%int8 pin~%float32 state #STATE_OFF or STATE_ON value, or other value (if type expressed in a specific control module)~%~%~%"))
