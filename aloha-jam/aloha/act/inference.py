...
def model_inference(args, config, ros_operator, save_episode=True):
    ...
    # 发布基础的姿态
    left0 = [0.47526852708386785, 0.48169530071596073, 1.8561132912322087, -0.7557240906883221, 1.4351688561701588, 0.3424517566735996, 1]
    right0 = [-0.00133514404296875, 0.00438690185546875, 0.034523963928222656, -0.053597450256347656, -0.00476837158203125, -0.00209808349609375, 1]
    left1 = [0.47526852708386785, 0.48169530071596073, 1.8561132912322087, -0.7557240906883221, 1.4351688561701588, 0.3424517566735996, 1]
    right1 = [-0.00133514404296875, 0.00247955322265625, 0.01583099365234375, -0.032616615295410156, -0.00286102294921875, 0.00095367431640625, 1]
...
class RosOperator:
    ...
    def puppet_arm_publish(self, left, right):
        ...
        try:
            ...
            if left[6] == 1: request.state = SetIORequest.STATE_ON
            else:
    ...
    def feedback_callback(self, msg):
        ...
        joint_pos_with_gripper = list(msg.joint_pos) + [msg.cb_digital_output[0]]
...
def get_arguments():
    ...
    parser.add_argument('--img_front_topic', action='store', type=str, help='img_front_topic',
                        default='/depth_camera_gripper/rgb_image', required=False)
    parser.add_argument('--img_left_topic', action='store', type=str, help='img_left_topic',
                        default='/depth_camera_gripper/rgb_image', required=False)
    parser.add_argument('--img_right_topic', action='store', type=str, help='img_right_topic',
                        default='/depth_camera_right/rgb_image', required=False)
    
    parser.add_argument('--img_front_depth_topic', action='store', type=str, help='img_front_depth_topic',
                        default='/depth_camera_gripper/depth_image', required=False)
    parser.add_argument('--img_left_depth_topic', action='store', type=str, help='img_left_depth_topic',
                        default='/depth_camera_gripper/depth_image', required=False)
    parser.add_argument('--img_right_depth_topic', action='store', type=str, help='img_right_depth_topic',
                        default='/depth_camera_right/depth_image', required=False)
